...
folder are mapped to the application's root namespace, 'cosmoTheme'. This is set in
`"app.json"`:

    "sass": {
        "namespace": "cosmoTheme"
    }
...
included in the application's JavaScript code build. By default, files in this 
folder are mapped to the application's root namespace, 'cosmoTheme' in the same way
as `"cosmoTheme/sass/src"`.
